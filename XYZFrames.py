...
class XYZ():
    def __init__(self):
        ...
        self.HSVs = {"center": (104, 255, 120), "x": (
            173, 231, 116), "y": (67, 255, 82)}
    ...
    def configurationModeUpdate(self, frame, pos, tollerance=10, useAverage=False, calculateYOffset=False, returnMasks=False):
        ...
        frame, masks = FrameOperations.findMasks(
            frame, *order, tolllerance=tollerance)
        for i in masks:
        ...
        if validread:
            ...
            if calculateYOffset:
                self.calculateYOffset(
                    *self.savedRead, centers[0], centers[1], pos[2])
            self.configureCameraCalibation(*centers, pos)
            self.pastRead = centers[0], centers[1], pos[2]
            self.centerScreenOffset = np.array(self.pointToCoroodinate(
                [i/2 for i in self.frameDims], pos))-pos[:2]
        if returnMasks:
    ...
    def getMaxViewDims(self, height):
        "retruns the maximum x and y dimentions the camera can see"
        maxx, maxy = np.array(self.frameDims) * \
            self.scalefactor*(height+self.yoffset)
        maxx, maxy = FrameOperations.rotatedRectWithMaxArea(
            maxx, maxy, self.angle)
        return maxx, maxy

    def createViewRenderer(self, maxx, maxy, height):
        "creates a funtion that draws the maximum x and y dimentions to a frame"
        # create renderer
    ...
    def create_auto_path(self, frame, maskkwargs: dict, centerkwargs: dict) -> Generator:
        "creates a generator object, takes in frame, mask from the .send fuction retruns frame, [x,y,z,claw]"
        # wants to have the maximum view for the camera
        maxx, maxy = self.getMaxViewDims(self.printerdims[2])
        dimentionDrawer = self.createViewRenderer(
            maxx, maxy, self.printerdims[2])
        detected_points = []
        for i in self.create_search_path(maxx, maxy):
            for n in range(5):  # scan 5 frames per point
                # draw the available viewing box
                frame = dimentionDrawer(frame)
                frame = yield frame, i
                frame, mask = FrameOperations.maskImage(frame, **maskkwargs)
                frame, detectedCenterPoint = FrameOperations.findCentre(
                    frame, mask, **centerkwargs)
                if detectedCenterPoint != None:
                    detected_points.append(detectedCenterPoint)
            if detected_points:
                coordinate = self.calculateAverageCordiante(
                    detected_points, i[:2]+(self.printerdims[2],), centerofscreen=True)
                detected_points = []
                for n in range(10):  # 10 tries per "sure" frame
                    frame = yield frame, (coordinate + [None, None])
                    frame, mask = FrameOperations.maskImage(
                        frame, **maskkwargs)
                    frame, detectedCenterPoint = FrameOperations.findCentre(
                        frame, mask, **centerkwargs)  # make sure you're centered
                    if detectedCenterPoint != None:
                        detected_points.append(detectedCenterPoint)
                if detected_points:
                    coordinate = self.calculateAverageCordiante(
                        detected_points, coordinate+[self.printerdims[2]])
                    break
        else:
    ...
    def pointToCoroodinate(self, point, printerpos, centerofscreen=False):
        ...
        scaled = FrameOperations.scalePoint(
            rotated, self.scalefactor*(printerpos[2]+self.yoffset), framecenter)
    ...
    def coroodinateToPoint(self, coordinate, printerpos):
        "given a real world cooriate, retrun the corisponding pixel values"
        if len(coordinate) == 3:
        ...
        frameCenter = [i/2 for i in self.frameDims]
        scaled = FrameOperations.scalePoint(
            translated, 1/(self.scalefactor*(printerpos[2]+self.yoffset)), frameCenter)
        rotated = FrameOperations.rotatePoints(
            scaled, -self.angle, frameCenter)
    ...
    def configureCameraCalibation(self, pointunderclaw, highxpoint, highypoint, pos):
        ...
        frameCenter = [i/2 for i in self.frameDims]
        pointunderclaw = FrameOperations.scalePoint(
            pointunderclaw, self.scalefactor*(pos[2]+self.yoffset), frameCenter)
        highxpoint = FrameOperations.scalePoint(
            highxpoint, self.scalefactor*(pos[2]+self.yoffset), frameCenter)
        highypoint = FrameOperations.scalePoint(
            highypoint, self.scalefactor*(pos[2]+self.yoffset), frameCenter)
        difference = highxpoint-pointunderclaw  # now rotated around 0,0
        # now all values should be between -1 and 1, for sin and cos
        difference = difference/expecteddisstance
        ...
        if np.isnan(self.angle):
            ...
            self.angle = np.pi
        pointunderclaw = FrameOperations.rotatePoints(
            unaffected, self.angle, frameCenter)
        pointunderclaw = FrameOperations.scalePoint(
            pointunderclaw, self.scalefactor*(pos[2]+self.yoffset), frameCenter)
        # alighn with real claw incase of offset
        pointunderclaw = pointunderclaw + self.dot_offset
