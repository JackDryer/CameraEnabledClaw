...
class App():
    ...
    def update(self):
        ...
        self.loctext.set("X:{0}, Y:{1}, Z:{2}, Claw:{3}".format(
            *[round(i, 2) for i in self.printer.get_current_pos()]))
        if self.cap.isOpened():
            ...
            if ret == True:
                ...
                if self.mode.get() == 0:  # manual mode
                    self.update_pos()
                ...
                elif self.mode.get() == 1:  # automatic mode
                    if self.auto_path is None:
                        self.auto_path = self.xyz.create_auto_path(
                            self.frame, self.autoMaskConfigs, self.autoCenterConfigs)
                        self.frame, pos = next(self.auto_path)
                        self.printer.add_next(*pos)
                    else:
                        if self.printer.command_is_finished():
                            try:
                                self.frame, pos = self.auto_path.send(
                                    (self.frame))
                                self.printer.add_next(*pos)
                    ...
                    if self.printer.get_current_pos()[2]:
                        self.drawArrows()
                    self.render(self.frame)
                elif self.mode.get() == 2:  # calibate mode
                    self.update_pos()
                    if self.calculateYBool.get() and not hasattr(self.xyz, "savedRead"):
                        messagebox.showerror(
                            message='You need to have previously set a "good read"')
                        self.calculateYBool.set(0)
                    validread, masks = self.xyz.configurationModeUpdate(self.frame, self.printer.get_current_pos(), tollerance=self.tollerance.get(
                    ), useAverage=self.useAverage.get(), calculateYOffset=self.calculateYBool.get(), returnMasks=True)
                    if validread:
                        ...
                        cv2.circle(self.frame, self.xyz.coroodinateToPoint(
                            dotpos[:2], printerpos=pos), 10, CENTERDOTCOLOUR, -1)
                        cv2.circle(self.frame, self.xyz.coroodinateToPoint(
                            (dotpos[0], dotpos[1]+10), printerpos=pos), 10, YDOTCOLOUR, -1)
                        cv2.circle(self.frame, self.xyz.coroodinateToPoint(
                            (dotpos[0]+10, dotpos[1]), printerpos=pos), 10, XDOTCOLOUR, -1)
                ...
                elif self.mode.get() == 3:  # get hsvs mode
                    masks = []
                    for i in self.HSVdisplayers.values():
                        if i.render.get():
                            frame, mask = FrameOperations.findMasks(
                                self.frame, i.colour, tolllerance=self.tollerance.get())
                            masks.append(mask[0])
                    self.render(self.frame, *masks)
                elif self.mode.get() == 4:  # configure atuo mode
                    ...
                    self.frame, mask = FrameOperations.maskImage(
                        self.frame, **self.autoMaskConfigs)
                    self.frame, centerpoint = FrameOperations.findCentre(
                        self.frame, mask, **self.autoCenterConfigs)
    ...
    def get_pixel(self, event: tk.Event):
        "given a click event on the canvas, return the corisponding y and x coordinates of the frame"
    ...
    def get_pixel_colour(self, event: tk.Event, colourtype=cv2.COLOR_BGR2HSV, drawcirlce=False):
        ...
        if drawcirlce:
            ...
            # this is cursed but for some reason it wroks
            color = (int(color[0]), int(color[1]), int(color[2]))
            # so you're sure you've selected the right one
            cv2.circle(self.frame, (x, y), 20, color, -1)
    ...
    def setSize(self, event: tk.Event):
        "resize the image to fill the space when resizing the canvas"
        if self.numframes == 2:
        ...
        if self.vidwidth == 0 or self.vidheight == 0:
            ...
            if newratio > oldratio:  # the Relative width is larger than needed
                # so scale the width the same as the height
                self.vidwidth = self.vidwidth * (event.height/self.vidheight)
                self.vidheight = event.height
            else:
    ...
    def render(self, *frames, numframes=1):
        "diplay up to 4 frames on the screen"
        if self.cap.isOpened() == False:
            ##ratio = self.vidheight/self.vidwidth
            frame = np.random.randint(0, 255, (200, 200, 3), np.uint8)
            frame = cv2.resize(frame, (int(self.vidwidth), int(
                self.vidheight)), interpolation=cv2.INTER_CUBIC)
            ...
            frame = cv2.resize(frame, (int(width), int(
                self.vidheight)), interpolation=cv2.INTER_CUBIC)
            ...
            scaleddown = cv2.resize(
                frames[0], None, fx=1/5, fy=1/5, interpolation=cv2.INTER_CUBIC)
            while len(frames) < 4:
                frames.append(np.random.randint(
                    0, 255, scaleddown.shape, np.uint8))
                frames[-1] = cv2.resize(frames[-1], frames[0].shape[1::-1],
            ...
            self.render(cv2.resize(full, None, fx=0.5, fy=0.5,
                        interpolation=cv2.INTER_CUBIC), numframes=4)
    ...
    def reload_video(self, *args):
        ...
        if (self.cap.isOpened() == False):
            ...
            self.xyz.frameDims = [int(self.cap.get(cv2.CAP_PROP_FRAME_WIDTH)), int(
        ...
        if self.canvas.winfo_width()+self.canvas.winfo_height() > 2:  # returns 1 if unupdated
            self.setSize(holder(self.canvas.winfo_width(),
                         self.canvas.winfo_height()))

    def drawArrows(self):
        "draws arrows corrispoding to the 10 cm in both the x and y starting at the point direcly below the claw"
        pos = self.printer.get_current_pos()
        center = self.xyz.coroodinateToPoint(pos[:2], printerpos=pos)
        highx = self.xyz.coroodinateToPoint(
            (pos[0]+10, pos[1]), printerpos=pos)
        highy = self.xyz.coroodinateToPoint(
            (pos[0], pos[1]+10), printerpos=pos)
    ...
    def make_modes(self):
        ...
        self.maunalbutton = tk.Radiobutton(
            self.modesframe, var=self.mode, value=0, text="Manual Control")
        self.autobutton = tk.Radiobutton(
            self.modesframe, var=self.mode, value=1, text="Auto")
        self.calibatebutton = tk.Radiobutton(
            self.modesframe, var=self.mode, value=2, text="Calibate")
        self.gethsvbutton = tk.Radiobutton(
            self.modesframe, var=self.mode, value=3, text="Get HSV")
        self.configautobutton = tk.Radiobutton(
            self.modesframe, var=self.mode, value=4, text="Configure automatic mode")
    ...
    def make_auto_config(self):
        ...
        self.autoMaskConfigs = {"colour": (
            0, 255, 150), "tollerance": 10, "closings": 1, "openings": 1, "dilateions": 0}
        ...
        self.autoconfigWidgets["colour"] = tk.Button(
            ...
            text="Colour",
            background='#%02x%02x%02x' % tuple((cv2.cvtColor(
                np.uint8([[self.autoMaskConfigs["colour"]]]), cv2.COLOR_HSV2RGB))[0][0]),
        ...
        for i in ("tollerance", "closings", "openings", "dilateions", "minradius"):
            ...
            self.autoconfigWidgets[i] = tk.Scale(
                self.autoModeConfigFrame, variable=self.autoconfigVars[i], to=to, orient=HORIZONTAL, label=i, length=150)
    ...
    def update_auto_mask_colour(self, event: tk.Event):
        ...
        self.autoconfigWidgets["colour"].config(
            bg='#%02x%02x%02x' % self.get_pixel_colour(event, cv2.COLOR_BGR2RGB))
    ...
    def make_manuals(self):
        ...
        self.setboxbutton = tk.Button(
            self.manualControlsFrame, text="set box pos", command=self.set_box_pos)
        self.setboxbutton.grid(row=0, columnspan=2, sticky="NSEW")
        self.useFrameCenter = tk.IntVar(value=0)
        self.useClawCenterButton = tk.Radiobutton(
            self.manualControlsFrame, variable=self.useFrameCenter, value=0, text="Claw Center")
        self.useFrameCenterButton = tk.Radiobutton(
            self.manualControlsFrame, variable=self.useFrameCenter, value=1, text="Frame Center")
    ...
    def make_camera_controls(self):
        ...
        self.pausebox = tk.Checkbutton(
            self.camracontrolframe, text="Pause Camera", variable=self.paused)
        self.pausebox.grid(sticky="SW")
        self.blured = tk.IntVar()
        self.blurbox = tk.Checkbutton(
            self.camracontrolframe, text="Blur Camera", variable=self.blured)
    ...
    def make_controls(self):
        ...
        namestokeys = {"right": "Right", "left": "Left", "forward": "Up",
                       "backward": "Down", "raise": "w", "lower": "s", "open": "o", "close": "c"}
        ...
        self.homebutton = tk.Button(
            self.controls_frame, text="Home All", command=self.printer.home)
        self.homebutton.grid(row=5, column=0, columnspan=4, sticky="NSEW")
        for i in range(4):
            self.controls_frame.grid_columnconfigure(i, weight=1)

    def update_pos(self):
        "Called within the main loop, allows the printer to move multiple steps without haveing to click the buttons multiple times"
    ...
    def set_box_pos(self):
        "sets the box position to either under the claw or to under the center of the screen depending on the mode"
        if self.useFrameCenter.get():
            if self.xyz.isCalibrated:
                self.xyz.boxpos = self.xyz.pointToCoroodinate(
                    [i/2 for i in self.xyz.frameDims], self.printer.get_current_pos(), True)
            else:
    ...
    def renderMousePos(self, event: tk.Event):
        ...
        if tup is None:
            ...
            return
        y, x = tup
        pos = self.xyz.pointToCoroodinate(
            (x, y), self.printer.get_current_pos(), centerofscreen=self.useFrameCenter.get())
        pos = [i.round(2) for i in pos]

        def closeure():
            self.frame = cv2.putText(self.frame, str(
                pos), (x, y), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255))
    ...
    def goToClick(self, event: tk.Event):
        if self.xyz.isCalibrated:
            y, x = self.get_pixel(event)
            coordinate = self.xyz.pointToCoroodinate(
                (x, y), self.printer.get_current_pos(), centerofscreen=self.useFrameCenter.get())
    ...
    def make_HSVs(self):
        ...
        self.tolleranceSlider = tk.Scale(
            self.HSVframe, to=255, variable=self.tollerance, orient=tk.HORIZONTAL, label="Tollerance Value", length=150)
        ...
        class HSVdisplayer:
            def __init__(self, master: tk.Frame, name, variable, colour=(0, 0, 0)):
                self.colour = colour
                self.name = name
                self.selector = tk.Radiobutton(
                    master, variable=variable, value=name, text=name)
                converted = tuple(
                    (cv2.cvtColor(np.uint8([[colour]]), cv2.COLOR_HSV2RGB))[0][0])
                self.colourbox = tk.Button(master, background='#%02x%02x%02x' % converted, height=1, width=1, command=lambda: print(
                    f"{self.name}: {self.colour}"))  # ,highlightthickness=2,highlightcolor="Black")
                self.render = tk.IntVar(value=0)
                self.renderbox = tk.Checkbutton(master, var=self.render)
        self.HSVdisplayers = {}
        for i in self.xyz.HSVs.keys():
            self.HSVdisplayers[i] = HSVdisplayer(
                self.HSVframe, i, self.HSVselector, self.xyz.HSVs[i])
            self.HSVdisplayers[i].selector.grid(
                row=len(self.HSVdisplayers), column=0, sticky="NW")
            self.HSVdisplayers[i].colourbox.grid(
                row=len(self.HSVdisplayers), column=1)
            self.HSVdisplayers[i].renderbox.grid(
                row=len(self.HSVdisplayers), column=2)
        self.calibrationpointstrings = [tk.StringVar(self.HSVframe, value="0", name="xDotoffset"), tk.StringVar(
            self.HSVframe, value="0", name="yDotoffset")]
        for i in self.calibrationpointstrings:
            i.trace("w", self.update_dot_offset)
        self.calibrationboxlables = []
        self.calibrationboxlables.append(
            tk.Label(self.HSVframe, text="Dot x offset"))
        self.calibrationboxlables.append(
            tk.Label(self.HSVframe, text="Dot y offset"))
        ...
        self.calibrationboxes.append(
            tk.Entry(self.HSVframe, textvariable=self.calibrationpointstrings[0], width=5))
        self.calibrationboxes.append(
            tk.Entry(self.HSVframe, textvariable=self.calibrationpointstrings[1], width=5))
        for x, i in enumerate(self.calibrationboxes):
            i.grid(row=len(self.HSVdisplayers)+x+1, column=1, sticky="NW")
        # allow for the boxes to be clicked out of
        self.root.bind_class("Frame", "<Button-1>",
                             lambda event: event.widget.focus_set())

    def update_dot_offset(self, varname, *args):
        "Update entered values forthe offset of the claw to the orienter"
        try:
    ...
    def handelHSVs(self, event: tk.Event):
        ...
        self.HSVdisplayers[self.HSVselector.get(
        )].colour = self.get_pixel_colour(event)
        self.HSVdisplayers[self.HSVselector.get()].colourbox.configure(
            bg='#%02x%02x%02x' % self.get_pixel_colour(event, cv2.COLOR_BGR2RGB))
    ...
    def make_Configurations(self):
        ...
        for x, i in enumerate(("offset", "angle", "scalefactor", "yoffset", "centerScreenOffset")):
            ...
            self.configReadOut[i].grid(
                row=x, column=0, sticky="nw", columnspan=2)
        self.saveConfigButton = tk.Button(
            self.configFrame, text="Good Read", command=self.xyz.saveRead)
        self.saveConfigButton.grid(row=5, column=0, sticky="nw")
        self.calculateYBool = tk.IntVar()
        self.calculateYBoolBox = tk.Checkbutton(
            self.configFrame, variable=self.calculateYBool, text="Calc. Y offset")
        self.calculateYBoolBox.grid(row=5, column=1, sticky="nw")
        self.useAverage = tk.IntVar()
        self.useAverageButton = tk.Checkbutton(
            self.configFrame, variable=self.useAverage, text="Calcuate Average Point", command=self.xyz.resetAverage)
    ...
    def configure_colours_recusrive(self, widget: tk.Widget):
        ...
        if isinstance(widget, tk.Frame):
            ...
            widget.configure(fg=FOREGROUNDCOLOUR,
                             highlightbackground=BACKGROUNDCOLOUR)
            if isinstance(widget, (tk.Checkbutton, tk.Radiobutton)):
                widget.config(selectcolor=SELECTCOLOUR,
                              activebackground=ACTIVEBACKGROUNDCOLOUR)
    ...
    def make_speeds(self):
        ...
        self.speedSlider = tk.Scale(self.speedFrame, to=20, variable=self.speed,
                                    orient=tk.HORIZONTAL, resolution=0.01, label="Control Speed", length=150)
        self.speedSlider.grid(row=0, column=0, sticky="NW")
        self.printerpercentspeed = tk.IntVar(value=100)
        self.printerspeedslider = tk.Scale(self.speedFrame, to=300, variable=self.printerpercentspeed,
                                           orient=tk.HORIZONTAL, label="Printer Speed Percent", length=150)
        self.printerspeedslider.grid(row=1, column=0, sticky="NW")
        self.sendspeedbutton = tk.Button(
            self.speedFrame, text="Override Printer Speed", command=self.overrideSpeed)
